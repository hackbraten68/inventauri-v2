...
- [ ] Ensure PocketBase admin credentials and Postgres seed data are provisioned so dashboards and inventory views populate.

**Project Snapshot**
- Astro + React TypeScript app with PocketBase auth, Tailwind UI, Prisma ORM, and Docker-first workflows advertised in the docs (`README.md:1`, `package.json:1`).
...
- Database modeled for multi-tenant inventory: shops, products, variants, warehouses, stock levels, and transactions plus PocketBase-linked user mappings (`prisma/schema.prisma:1`).
...
- Local dev expects `.env.local` with PocketBase + Postgres settings or `.env.docker` for Compose; dotenv wrapper is baked into every npm script (`package.json:4`, `tests/setup.ts:1`).

**Implemented Features**
- Public landing page, login screen with PocketBase password flow, and theme toggle components already wired up (`src/pages/login.astro:1`, `src/components/auth/LoginForm.tsx:1`).
...
- PocketBase credentials must exist (plus seeded Prisma data) or most UI panels will render empty states until the default shop/user mappings are created (`src/pages/api/auth/login.ts:1`, `prisma/seed.ts`).
